...
function turn(id: string, status: Turn["status"], compaction = false): Turn {
  const item = compaction
    ...
    : { type: "agentMessage" as const, id: `${id}-item`, text: id, phase: null, memoryCitation: null, delivery: null, questions: null };
  return {
...
function record(directory: string): ClaudeThreadRecord {
  return {
    thread: {
      ...
      preview: "compacted branch", ephemeral: false, section: null, sectionEnteredAt: null, environments: null, originator: null, daybreakEnabled: null, projectId: null, historyMode: "legacy", modelProvider: "claude", model: "claude:claude-fable-5", reasoningEffort: "high",
...
describe("compacted transcript rollback boundary", () => {
    ...
    brancher.afterForkValidated = () => {
      store.updateTurn("rollback-thread", {
        ...
        items: [{
          ...
          phase: null, memoryCitation: null, questions: null, delivery: null,
        }],
      });
    };
    const service = new ClaudeService(
  ...
  it("keeps an active /side fork pinned while the parent turn streams new items", async () => {
    ...
    brancher.afterForkValidated = () => {
      store.updateTurn("rollback-thread", {
        ...active,
        items: [{ type: "agentMessage", id: "late-item", text: "late parent delta", phase: null, memoryCitation: null, questions: null, delivery: null }],
      });
    };
    const service = new ClaudeService(
      config(directory), new SubscriptionHub(), new Logger("error"), store,
