...
describe("npm-backed hybrid daemon lifecycle", () => {
  it("matches Codex 0.157.1 wire shapes and cleans the detached process group", async () => {
    ...
    expect(wire(await run("enable-remote-control"))).toMatchObject({
      status: "enabled", backend: "pid", remoteControlEnabled: true, cliVersion: "0.157.1", appServerVersion: "0.157.1",
    });
    ...
    expect(wire(await run("restart"))).toMatchObject({ status: "restarted", backend: "pid", appServerVersion: "0.157.1" });
  ...
  it("replaces an unmanaged gateway that wins the readiness race", async () => {
    ...
    expect(result).toMatchObject({ status: "started", backend: "pid", appServerVersion: "0.157.1" });
  ...
  it("does not trust a live managed PID after the socket path is rebound", async () => {
    ...
    await expect(probeAppServer(config.publicSocket)).resolves.toMatchObject({ appServerVersion: "0.157.1" });
